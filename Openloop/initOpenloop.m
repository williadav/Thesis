...
Tg_step_N = [(T0g)*ones(1,200), (T0g-30)*ones(1,1800)];
Tg_step_P = [(T0g)*ones(1,200), (T0g+30)*ones(1,1800)];
...
p_in    = 24;               % [bar] Cold side inlet pressure NOT IN USE 
...
Kv      = 21.9/(2*0.9);     % [kg/bar] valve coeff for valve after OTSG
...
L       = 50;               % [kW] denoted as PL in code
mP0     = 21.9/2;           % [kg/s] desired pump output
m_cold_in = 10000;          % [kg/s] Condenser cold side mass flow
T_cold_in = 273.15+26.6769; % [K] Condenser cold side inlet temperature
k_c       = 21.9/2*100;     % [kg/(bar s)]  
p_out     = 0.0258;         % [bar] outlet pressure boundary
U_c       = 1;              % [kW/K] Condenser overall heat transfer coefficient
A_c       = 700;            % [m2] Condenser area
Vc        = 5;              % [m3] Condenser volume
...
constants = [R, cpS, cpW, cpG, T_Ref, TB_Ref, T0g, Tp, mG, dHvap0, ...
    ...
    A_tot, A_corr, Kv, zV, Vs, phiD, Mg, Dg, w0, L, mP0, m_cold_in, ...
    T_cold_in, k_c, p_out, U_c, A_c, Vc, A_anto_c, B_anto_c, C_anto_c, ...
    eta, VT];
...
m_cold_out_guess = m_cold_in;                          % [kg/s]
T_cold_out_guess = T_cold_in+0.2;                      % [K]
Qc_guess    = m_cold_out_guess*cpW*(T_cold_out_guess-T_cold_in); % [kW]

Tc_guess    = T_cold_in;                               % [K]
m_c_guess   = mP;                                      % [kg/s]
pc_guess    = 0.0358;                                  % [bar]
hc_guess    = cpW*(Tc_guess-T_Ref);                    % [kJ/kg]
rho_c_guess = 999.9;                                   % [kg/m3]
Mc_guess    = 0.1;                                     % [kg]
Hc_guess    = Mc_guess*hc_guess;                       % [kJ]
...
if ~isempty(load_from_file) && Hex_only
    ...
    z_0 = [z_0; m_s_guess; Ts_guess; TBs_guess; pS; hs_guess; beta_s_guess; ...
        ...
        Tc_guess; m_c_guess; pc_guess; hc_guess; rho_c_guess; ...
        m_cold_out_guess; T_cold_out_guess; Qc_guess];
...
elseif ~isempty(load_from_file) && ~Hex_only
    ...
    num_z = 10*n+1+30; % Number of z variables (hex + inlet + other)
...
%% Run
T = 2000; % [s] Total simulation time
N = T;   % [-] Number of steps 
...
for i = 1:N
    ...
    % Evaluate system 
    [xf_keys, zf_keys, result] = Openloop(n, x, z, constants); 
    x = result.xf;
    z = result.zf;
