...
control = [control; m_in_SP_0; z_m_0; KI_m_in; Ts_SP; Kc_Ts; tauI_Ts; v_0; ...
    pS_SP; zV_0; Kc_pS; tauI_pS; tauT_pS];
...
if ~isempty(load_from_file) && Hex_only
    ...
    x_0 = [x_0, interrmIn_guess, interrTs_guess, interrpS_guess,interrzV_guess];
...
elseif ~isempty(load_from_file) && ~Hex_only
    ...
    % System variables
    num_x = 2*n+10;     % Number of x variables
...
for i = 1:N
    ...
    % Evaluate system 
    [xf_keys, zf_keys, result] = C3FeedbackFeedforward(n, x, z, constants, control); 
    x = result.xf;
    z = result.zf;
