...
Tg_step_N = [(T0g)*ones(1,200), (T0g-10)*ones(1,1800)];
Tg_step_P = [(T0g)*ones(1,200), (T0g+10)*ones(1,1800)];
...
control = [control; m_in_SP; z_m_0; KI_m_in];
...
if ~isempty(load_from_file) && Hex_only
    ...
    z_0 = [z_0; dummy_zm_guess];
    x_0 = [x_0, interrmIn_guess];
...
elseif ~isempty(load_from_file) && ~Hex_only
    ...
    % System variables
    num_x = 2*n+7;     % Number of x variables
    num_z = 10*n+1+27; % Number of z variables (hex + inlet + other)
...
for i = 1:N
    ...
    % Evaluate system 
    [xf_keys, zf_keys, result] = FlowController(n, x, z, constants, control); 
    x = result.xf;
    z = result.zf;
