function [x_keys, z_keys, sol] = ...
    CombinedCycleCase(n, x_0, z_0, constants, control)
    import casadi.*
    ...
    interrzVVPC = SX.sym('interrzVVPC');
    interrupS_SP = SX.sym('interrupS_SP');
    dummy_pS_SP_VPC = SX.sym('dummy_pS_SP_VPC');
    ...
    % Control
    x = [x; interrmIn;interrTs;interrpS;interrzV;interrzVVPC;interrupS_SP];
    ...
    % Control
    z = [z; dummy_zm; dummy_m_in_SP; dummy_v; dummy_zV; dummy_mg;...
        dummy_pS_SP; dummy_pS_SP_VPC];
    ...
    dHvap = dHvap0 + (cpW-cpS)*(TB_Ref-Tp);   % Evaporation enthalpy at Tp
    ...
    % Equations
    % VPC 
    errzVVPC = zV_0 - dummy_zV;       % err VPC input (zV_SP-zV_APPL) DUMMY_ZV IS ZV_APPL 
    pS_SP_VPC = pS_SP_0 + Kc_VPC*errzVVPC + Kc_VPC/tauI_VPC*interrzVVPC + 1/tauT_VPC*interrupS_SP; 
    pS_SP = fmax(pS_min, fmin(pS_SP_VPC, pS_max)); % Mid selector
    errupS_SP = pS_SP - pS_SP_VPC;                 % for anti-windup
    ...
    Dummy7 = dummy_pS_SP_VPC - pS_SP_VPC;
    
    Alg = [Alg; Dummy1; Dummy2; Dummy3; Dummy4; Dummy5; Dummy6; Dummy7];

    % Add error to ode
    diff = [diff; errmIn; errTs; errpS; errzV; errzVVPC; errupS_SP]; 
    ...
    %x_sol = full(sol.xf);   % Final differential states
    %z_sol = full(sol.zf);   % Final algebraic states
    % Suppose Alg is a CasADi expression in x, z, and t.
    % Create a function to evaluate the algebraic residuals
    %alg_fun = Function('alg_fun', {x, z}, {Alg});
    
    % Evaluate at final time (e.g., t = 1 if tf=1)
    %res = full(alg_fun(x_sol, z_sol));
    
    %disp('Residuals of algebraic equations:');
    %disp(res);
