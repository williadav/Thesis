...
L       = 11;               % [kW] denoted as PL in code CHANGE THIS
...
k_m      = 21.9/(2*0.5*(p_pump-p_guess(1))); % [kg/bar]%10.66939/0.5;   % pump valve coeff designed for 10.95 with z =0.5
...
control = [control; m_in_SP_0; z_m_0; KI_m_in; Ts_SP; Kc_Ts; tauI_Ts; v_0; ...
    pS_SP; zV_0; Kc_pS; tauI_pS; tauT_pS; Kc_VPC; tauI_VPC; tauT_VPC; ...
    pS_min; pS_max];
...
interrzVVPC_guess = 0;
interrupS_SP_guess = 0;
dummy_pS_SP_VPC_guess = pS;
dummy_zV_guess = zV;
...
if ~isempty(load_from_file) && Hex_only
    ...
    z_0 = [z_0; dummy_zm_guess; dummy_m_in_SP; v_guess; dummy_zV_guess;...
        dummy_mg_guess; dummy_pS_SP_guess; dummy_pS_SP_VPC_guess];
    x_0 = [x_0, interrmIn_guess, interrTs_guess, interrpS_guess,...
        interrzV_guess, interrzVVPC_guess, interrupS_SP_guess];
...
elseif ~isempty(load_from_file) && ~Hex_only
    ...
    % System variables
    num_x = 2*n+12;     % Number of x variables
    num_z = 10*n+1+33; % Number of z variables (hex + inlet + other)
...
for i = 1:N
    ...
    % Evaluate system 
    [xf_keys, zf_keys, result] = CombinedCycleCase(n, x, z, constants, control); 
    x = result.xf;
    z = result.zf;
