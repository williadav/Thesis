...
control = [control; m_in_SP_0; z_m_0; KI_m_in; Ts_SP; Kc_Ts; tauI_Ts; v_0; ...
    pS_SP; zV_0; Kc_pS; tauI_pS; tauT_pS; W_SP; Kc_W1; Kc_W2; tauI_W2];
...
if ~isempty(load_from_file) && Hex_only
    ...
    z_0 = [z_0; dummy_zm_guess; dummy_m_in_SP; v_guess; dummy_pS_guess;...
        dummy_mg_guess];
    x_0 = [x_0, interrmIn_guess, interrTs_guess, interrW_guess];
...
elseif ~isempty(load_from_file) && ~Hex_only
    ...
    % System variables
    num_x = 2*n+9;     % Number of x variables
    num_z = 10*n+1+31; % Number of z variables (hex + inlet + other)
...
for i = 1:N
    ...
    % Apply steps 
    control(step_index{y}) = applied_step{y}(i);  % Assign corresponding value at index

    % Evaluate system 
    [xf_keys, zf_keys, result] = SteamCycleCaseFloating(n, x, z, constants, control); 
    x = result.xf;
    z = result.zf;
